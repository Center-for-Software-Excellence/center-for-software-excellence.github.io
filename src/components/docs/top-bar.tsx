  ...
  return (
    ...
    >
      ...
      {showSidebar && (
        <span
          className={cn(
            'animate-expand-y fixed top-0 bottom-0 left-70 hidden h-screen w-px bg-[linear-gradient(to_top,transparent_0%,var(--border)_5%,var(--border)_95%,transparent_100%)] transition-all duration-300 ease-in-out md:block',
          )}
          aria-hidden
        />
      )}
      {/* fake topbar border */}
      <span
        className="animate-expand-x absolute inset-x-0 bottom-0 hidden h-px bg-[linear-gradient(to_right,transparent_0%,var(--border)_10%,var(--border)_90%,transparent_100%)] md:block"
        aria-hidden
      />
